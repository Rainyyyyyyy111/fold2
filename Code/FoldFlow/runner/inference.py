...
from foldflow.guidance.energy_guidance import BinderEnergyGuidance
...
class Sampler:
    ...
    def sample(self, sample_length: int, context: Optional[torch.Tensor] = None):
        ...
        init_feats = tree.tree_map(
            lambda x: x if torch.is_tensor(x) else torch.tensor(x), init_feats
        )
        init_feats = tree.tree_map(lambda x: x[None].to(self.device), init_feats)
        ...
        return tree.tree_map(lambda x: x[:, 0], sample_out)

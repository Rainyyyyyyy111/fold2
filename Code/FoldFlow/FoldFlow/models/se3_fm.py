...
class SE3FlowMatcher:
    ...
    def reverse(
        ...
        if guidance_fn is not None:
            ...
            # We must convert inputs to torch tensors for autograd
            device = 'cuda' if torch.cuda.is_available() else 'cpu'
            rot_tensor = torch.tensor(rot_t, dtype=torch.float32, device=device)
            trans_tensor = torch.tensor(trans_t, dtype=torch.float32, device=device)
            
            grad_rot, grad_trans = guidance_fn.get_guidance_gradients(rot_tensor, trans_tensor)
            
            # --- Adaptive Cosine-Similarity Throttling (Ghost Manifold Defense) ---
            # Calculate cosine similarity between the model's predicted vector field and the physical guidance.
            # If they are pointing in opposite directions (cos_sim < 0), it means the physical guidance 
            # is forcing the structure into an Out-of-Distribution "Ghost Manifold" that cannot be folded.
            # We throttle the guidance to preserve the biological prior.
            trans_vf_tensor = torch.tensor(trans_vectorfield, dtype=torch.float32, device=device)
            cos_sim = F.cosine_similarity(trans_vf_tensor.flatten(1), grad_trans.flatten(1), dim=1)
            # Map cos_sim from [-1, 1] to [0.0, 1.0] as a throttling factor.
            throttle_factor = torch.clamp((cos_sim + 1.0) / 2.0, min=0.0, max=1.0).view(-1, 1, 1)
            
            # --- Time Annealing (Catch-22 Fix) ---
            # Flow matching t goes from 1.0 (pure noise) to 0.0 (clean structure).
            # Calculating steric clash / physical energy on noise is meaningless and destroys the flow trajectory.
            # We use an annealing scale (1.0 - t)^2 so guidance is 0 at t=1 and fully active at t=0.
            anneal_scale = (1.0 - t) ** 2
            
            final_scale = (anneal_scale * throttle_factor).detach().cpu().numpy()
            
            # The model outputs a vector field. We "push" the vector field in the direction of steepest energy descent.
            # Convert back to numpy to match the expected shapes for the integrators.
            rot_vectorfield = rot_vectorfield + final_scale * grad_rot.detach().cpu().numpy()
            trans_vectorfield = trans_vectorfield + final_scale * grad_trans.detach().cpu().numpy()
